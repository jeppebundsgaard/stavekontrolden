...
if(!$c["r_description"]) {
	...
	//Status set to 2 when word created
		$q='insert into affixrule (`lang`,`description`,`affixclassid`,`affix`,`condition`,`stripchars`,`replacement`,`morphdescrid`) values ("'.$_SESSION["lang"].'","'.$c["r_description"].'",'.$c["affixclassid"].',"'.$c["affix"].'","'.$c["condition"].'","'.$c["stripchars"].'","'.$c["replacement"].'",'.($c["morphdescrid"]?$c["morphdescrid"]:0).')'; 
	...
	$res["newaffixrule"]=$c["newaffixrule"];
	$res["affixruleid"]=$c["affixruleid"];
	$res["description"]=$c["r_description"];
}
