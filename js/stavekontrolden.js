...
// 	console.log(txt)
	if(txt.length>9) {
		...
		$("#stavekontroldenWarningTxt").html(txt)
		$("#stavekontroldenWarning").show().delay(time).fadeOut()
		$("#closew").unbind("click").click(function() {$("#stavekontroldenWarning").hide()})
	}
}

function showMessage(txt,time) {
	if(typeof(time)=="undefined") var time=3000
	$("#stavekontroldenMessageTxt").html(txt)
	$("#stavekontroldenMessage").show().delay(time).fadeOut()
	$("#closem").unbind("click").click(function() {$("#stavekontroldenMessage").hide()})

}

function showError(txt,time) {
	if(typeof(time)=="undefined") var time=3000
	$("#stavekontroldenErrorTxt").html(txt)
	$("#stavekontroldenError").show().delay(time).fadeOut()
	$("closee").unbind("click").click(function() {$("#stavekontroldenError").hide()})
